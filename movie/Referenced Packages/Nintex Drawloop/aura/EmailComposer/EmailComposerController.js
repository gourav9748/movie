({
                ...
                if (response.getState() === "SUCCESS") {
                    ...
                    if ('template' in emailInfo) {
                        ...
                        if ('templateRendererUrl' in emailInfo) {
                            ...
                            var subdomain = hostname.substring(0, hostname.indexOf('.')) + '--loop';
                            var templateRendererUrl = emailInfo.templateRendererUrl.replace(/(https:\/\/)c(.+)/, '$1' + subdomain + '$2');
                        ...
                        if (emailInfo.template.Body) {
                            if ('templateBodyLines' in emailInfo) {
                                component.set('v.body', emailInfo.templateBodyLines.join('<br/>'));
                            } else {
                                component.set('v.body', emailInfo.template.Body);
                            }
                        }
                    }
    ...
    updateBody : function(component, event) {
        component.set('v.body', event.getSource().get('v.value'));
    },
    updateHtmlBody : function(component, event) {
        component.set('v.templateHtmlBody', event.getSource().get('v.value'));
    },
